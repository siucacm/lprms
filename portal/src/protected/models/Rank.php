 ...
 *
 * The followings are the available columns in table 'event_attendee_type':
 * @property integer $id
...
class Rank extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('prefix, description', 'required'),
			array('prefix', 'length', 'max'=>2),
			array('description', 'length', 'max'=>64),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, prefix, description', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'refUserEvents' => array(self::HAS_MANY, 'RefUserEvent', 'id_designation'),
		);
	}
