 ...
 * @author Sarah
 */
public class Panel_ItemList extends Panel {

    /** Creates new form Panel_UserList */
    public Panel_ItemList() {
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(lprms.LPRMSApp.class).getContext().getResourceMap(Panel_ItemList.class);
    ...
    private class RefreshTask extends Task {
        ...
        @Override protected Void doInBackground() {
            ...
            SQL_Library.load_actualItems();
            ArrayList<Item> items = SQL_Library.getActualItems();
            for (int i = 0; i < items.size(); i++)
            {
                data.addRow(items.get(i).getItemInfo());
                this.setProgress(i, 0, items.size());
            }
            info.setText("Number of items: "+items.size());
            return null;
        }
    ...
    public void processInput(String input)
    {
        ...
        int iid = 0;
        try {
            iid = Integer.parseInt(input.substring(1));
        } catch (NumberFormatException e) { System.err.println("Invalid ID!"); }
        if (iid != 0)
        {
            int row = lprms.sql.SQL_Library.findItem(iid);
            if (row != -1) 
            {
    ...
    public static final String[] COLS = {"ID", "Owner", "Borrower", "Status"};
}
